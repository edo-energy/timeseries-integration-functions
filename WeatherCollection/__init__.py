...
def main():
    ...
    for index, station in df_stations.iterrows():
        ...
        for index, missing in df_dates.iterrows():
            ...
            # set the datetime index as utc relative and then convert to the local timezone
            df = df.tz_localize(tz='UTC').tz_convert(tz=timezone).tz_localize(tz=None)
    ...
    with db.cursor() as cur:
        ...
        cur.execute("""CREATE TEMPORARY TABLE degreedays
                SELECT point.SiteID, regressionweatherdeppoints.PointID, p1.DateValue, '00:00:00' AS 'timevalue', IF (point.PointClassID = 622, IF(AVG(tr.NumericValue)-GET_NUMERIC(pointmetadata.MetadataValue) < 0, 0, AVG(tr.NumericValue)-GET_NUMERIC(pointmetadata.MetadataValue)), IF(GET_NUMERIC(pointmetadata.MetadataValue)-AVG(tr.NumericValue) < 0, 0, GET_NUMERIC(pointmetadata.MetadataValue)-AVG(tr.NumericValue))) AS 'NumericValue'
                FROM regressionweatherdeppoints
        ...
        cur.execute("""INSERT INTO point (SiteID,PointName, BldgID,PointClassID, TypeID) 
                                        ...
                                        JOIN regressionweatherstations ON (regressionweatherstationalias.WeatherStationID = regressionweatherstations.ID)
                                        JOIN point weather ON (regressionweatherstations.Link = LEFT(weather.PointName,4) AND weather.SiteID=140)
        ...
        cur.execute("""CREATE TEMPORARY TABLE weatheralias 
                                ...
                                JOIN regressionweatherstations ON (regressionweatherstationalias.WeatherStationID = regressionweatherstations.ID)
                                JOIN point weather ON (regressionweatherstations.Link = LEFT(weather.PointName,4) AND weather.SiteID=140)
                                JOIN pointdaily weatherdaily ON (weather.ID = weatherdaily.PointID)
                                JOIN tr ON (weather.SiteID = tr.SiteID AND weather.ID = tr.PointID AND weatherdaily.DateValue = tr.DateValue)
                                LEFT JOIN point ON (building.ID = point.BldgID AND weather.PointClassID = point.PointClassID AND point.Hidden=0)
                                LEFT JOIN pointdaily ON (point.ID = pointdaily.PointID AND weatherdaily.DateValue = pointdaily.DateValue)
                                WHERE (pointdaily.DateValue IS NULL OR pointdaily.NewRecords < weatherdaily.NewRecords) AND point.TypeID=7""")

        # insert then weather data into the alias points
        cur.execute("""INSERT INTO tr (SiteID,PointID,datevalue,timevalue,NumericValue) (SELECT * FROM weatheralias) ON DUPLICATE KEY UPDATE NumericValue = VALUES(NumericValue)""")
